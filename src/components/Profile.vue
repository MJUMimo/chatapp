<template>
  <div class="w-100 mt-3">
    <h2 class="fw-600">แนะนำตัวเองหน่อยสิ!</h2>
    <div class="profile">
    ...
    <div class="form-group">
      <label>เธอชื่ออะไร?</label>
      <input type="text" name="name" class="form-control" v-model="name" />
    </div>
    <div class="form-group">
      <label>เธอเป็นอย่างไรบ้าง?</label>
      <input type="text" name="aboutMe" class="form-control" v-model="aboutMe" />
    </div>
    <button type="button" class="btn btn-primary" v-on:click="uploadAvatar">บันทึก</button>
    <button type="button" class="btn btn-info" v-on:click="goBack">กลับสู่หน้าหลัก</button>
  </div>
</template>
